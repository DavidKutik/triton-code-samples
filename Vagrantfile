Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |v|
    ...
    # please adapt the used cores and memory to your needs
    v.cpus = 2
    v.memory = 4096

    # enable discard support that the guest can use fstrim
  ...
  # install basic package collection
  ...
  # install gcc-4.9 and other build relevant tools
  ...
  # symlink older gcc version to /usr/bin/gcc
  ...
  # install ancient kernel from launchpadlibrarian, needed for pin to work
  ...
  # build pin example
  config.vm.provision :shell, :inline => "cd /opt/pin-2.14-71313-gcc.4.4.7-linux/source/tools/ManualExamples && make -j$((`nproc`+1)) all TARGET=intel64"
end